...
export default function Reports() {
    ...
    //Alarms By Date
    const [startDate, setStartDate] = React.useState<Date | null>(parse('2025/01/01', 'yyyy/MM/dd', new Date()))
    const [endDate, setEndDate] = React.useState<Date | null>(parse('2026/01/01', 'yyyy/MM/dd', new Date()))
    const [alarmsByTime, setAlarmsByTime] = React.useState<[Alarm] | []>([]);
    ...
    //Tags By Date
    const [startDateTag, setStartDateTag] = React.useState<Date | null>(parse('2025/01/01', 'yyyy/MM/dd', new Date()))
    const [endDateTag, setEndDateTag] = React.useState<Date | null>(parse('2026/01/01', 'yyyy/MM/dd', new Date()))
    const [tagsByTime, setTagsByTime] = React.useState<any>([]);
    const [sortDirectionTagByDate, setSortDirectionTagByDate] = React.useState(true);

    //All Analog Tags
    const [allAnalogTags, setAllAnalogTags] = React.useState<any>([]);
    const [sortDirectionAllAnalogTags, setSortDirectionAllAnalogTags] = React.useState(true);


    //All Digital Tags
    const [allDigitalTags, setAllDigitalTags] = React.useState<any>([]);
    const [sortDirectionAllDigitalTags, setSortDirectionAllDigitalTags] = React.useState(true);

    //Alarms By Priority
    const [priority, setPriority] = React.useState("Normal")
    const [alarmsByPriority, setAlarmsByPriority] = React.useState<any>([]);
    ...
    const [tagsById, setTagsById] = React.useState<any>([]);
    const [myTags, setMyTags] = React.useState<any>([]);
    const [sortDirectionTagsById, setSortDirectionTagsById] = React.useState(true);

    function getAllAlarmsByTime() {
        axios.get(environment + `/api/Reports/allAlarmsByTime?timeFrom=` + format(startDate ?? Date(), "yyyy-MM-dd") + "&timeTo=" + format(endDate ?? Date(), "yyyy-MM-dd")).then(response => {
            ...
            if (sortAlarmByDate == "Priority")
                ...
                result.sort((a, b) =>direction*(parseISO(a.timestamp).getTime() - parseISO(b.timestamp).getTime()));
            console.log(result);
    ...
    function getAllTagsByDate(){
        axios.get(environment + `/api/Reports/allTagValuesByTime?timeFrom=` + format(startDateTag ?? Date(), "yyyy-MM-dd") + "&timeTo=" + format(endDateTag ?? Date(), "yyyy-MM-dd")).then(response => {
            const resultObj = response.data;
            const result=resultObj.analogData.concat(resultObj.digitalData)
    ...
    useEffect(()=>{
        axios.get(environment + `/api/Tag/getTags`).then(response=>{
            const resultObj = response.data;
